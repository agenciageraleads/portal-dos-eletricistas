    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col hover:shadow-md transition-shadow">
            <div className="h-40 bg-gray-50 rounded-lg mb-4 flex items-center justify-center text-gray-400 relative overflow-hidden">
                ...
                >
                    <div className="w-16 h-16 bg-brand-primary-light rounded-full flex items-center justify-center text-brand-primary">
                ...
                {product.specs && (
                    <div className="flex flex-wrap gap-1 mt-2 mb-1">
                        {product.specs.amperage && (
                            <span className="text-[10px] font-semibold bg-brand-primary-light text-brand-primary px-1.5 py-0.5 rounded border border-teal-100 uppercase">
                                {product.specs.amperage}
                            </span>
                        )}
                        {product.specs.curve && (
                        ...
                        {product.specs.gauge && (
                            <span className="text-[10px] font-semibold bg-brand-accent-light text-brand-accent px-1.5 py-0.5 rounded border border-cyan-100 uppercase">
                                {product.specs.gauge}
                            </span>
                        )}
            ...
            <div className="mt-2 text-right">
                <div className="flex items-baseline justify-end gap-1">
                    <p className="text-xl font-bold text-gray-900 tracking-tight">
                        {parseFloat(product.price) > 0 ? formatPrice(product.price) : <span className="text-sm text-brand-primary font-semibold">Sob Consulta</span>}
                    </p>
            ...
            <div className="mt-2 flex flex-col gap-2">
                ...
                {quantity > 0 ? (
                    <div className="flex items-center bg-gray-100 rounded-lg h-10 w-full overflow-hidden">
                        <button
                            aria-label="Diminuir quantidade"
                            className="px-3 h-full text-gray-600 hover:bg-gray-200 font-bold flex items-center justify-center min-w-[44px]"
                            onClick={(e) => {
                                e.stopPropagation();
                        ...
                        <input
                            ...
                            className="flex-1 h-full bg-transparent text-center font-medium text-sm outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none min-h-[44px]"
                            value={quantity}
                        ...
                        <button
                            ...
                            className="px-3 h-full text-brand-primary hover:bg-brand-primary-light font-bold flex items-center justify-center min-w-[44px]"
                            onClick={(e) => {
                                e.stopPropagation();
                                updateQuantity(product.id, quantity + 1);
                            }}
                    ...
                    <button
                        className="bg-brand-primary text-white h-11 px-4 rounded-lg hover:bg-brand-primary-hover transition-colors flex items-center justify-center gap-2 cursor-pointer active:scale-95 shadow-md w-full"
                        onClick={(e) => {
                            e.stopPropagation();
                            addToCart(product);
                        }}
